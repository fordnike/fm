...
.controller('mdcDatePickerInputController', function ($scope, $attrs, $timeout, $mdDialog) {
    $scope.$watch("model", function(a){
        console.log("checking model:" + a);

        if (!angular.isUndefined($scope.model)) {
            console.log("directive2: " + $scope.model);
            $scope.selected = {
                model: moment($scope.model).format('DD/MM/YYYY'),
                date: $scope.model
            };
        }
        else {
    ...
    console.log("directive1: " + $scope.model);
    ...
    $scope.openPicker = function (ev) {
      ...
      $mdDialog.show({
        targetEvent: ev,
        templateUrl: 'modules/shared/lib/date-picker/date-picker-dialog.html',
...
.directive('mdcDatePicker', function () {
    return {
      ...
      templateUrl: 'modules/shared/lib/date-picker/date-picker-input.html'
    };
  });
})();
(function(){'use strict';
